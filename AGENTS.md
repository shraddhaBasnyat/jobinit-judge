...
- `CarouselShell`'s `drag="x"` swipe gesture can be hijacked by a click on interactive stage content if the pointer drifts a few px mid-click (real bug, hit and fixed in Ticket 15 via a capture-phase `stopPropagation` guard — see the comment above `stopDragOnInteractive` in `CarouselShell.tsx`). That guard only recognizes a fixed selector list (`button`, `[role="radio"]`, `[role="checkbox"]`, links, inputs). A future interactive control built without one of those tags/roles (e.g. a bare `<div>` with just an `onClick`) falls outside it silently — extend the selector or give that control its own `onPointerDown={(e) => e.stopPropagation()}`.
